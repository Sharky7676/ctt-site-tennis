// Service API utilisant JsonBinStorage pour la synchronisation
class JsonBinApiService {
  ...
  async forceReload() {
    try {
      await this.storage.forceReload();
      return { success: true, message: 'Données rechargées depuis le serveur' };
    } catch (error) {
...
window.JsonBinApiService = JsonBinApiService;
